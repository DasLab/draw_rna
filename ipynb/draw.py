...
def draw_struct(seq, secstruct, c=None, line=False, large_mode=False, cmap='viridis', rotation=0):
    '''
    ...
    if c is not None:
        ...
        if isinstance(c[0], float):
            d.draw_rna(seq, secstruct, c, line=line, ext_color_file=True, cmap_name = cmap,
             rotation=rotation, large_mode = large_mode)
        else:
        ...
        d.draw_rna(seq, secstruct, seq2col(seq), line=line, cmap_name = cmap,
         large_mode = large_mode, rotation=rotation)